...
type FavoriteCardPickerProps = {
  cards: Card[];
  onClose: () => void;
  onConfirm: (cardIds: string[]) => Promise<void>;
};

export function FavoriteCardPicker({ cards, onClose, onConfirm }: FavoriteCardPickerProps) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      setError(t("pages.favourites.pickerAddError"));
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="flex h-[88vh] max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-lg border border-border bg-surface shadow-2xl">
        <div className="flex items-start justify-between gap-3 px-6 pb-3 pt-6">
          <div className="space-y-1">
            <h2 className="text-lg font-semibold">{t("pages.favourites.pickerTitle")}</h2>
            <p className="text-sm text-muted-foreground">{t("pages.favourites.pickerDescription")}</p>
          </div>
          <button
            type="button"
        ...
        <div className="px-6 pb-3">
          <label className="relative block">
            ...
            <input
              ...
              placeholder={t("pages.favourites.pickerSearchPlaceholder")}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6">
          {!cards.length ? (
            <div className="py-10 text-center text-sm text-muted-foreground">{t("pages.favourites.pickerEmpty")}</div>
          ) : !filteredCards.length ? (
            <div className="py-10 text-center text-sm text-muted-foreground">{t("pages.favourites.pickerNoMatch")}</div>
          ) : (
        ...
        <div className="flex items-center justify-between gap-2 border-t border-border px-6 py-4">
          <span className="text-sm text-muted-foreground">{t("pages.favourites.pickerSelected", { count: selectedIds.size })}</span>
          <div className="flex gap-2">
            ...
            >
              {saving ? t("pages.favourites.pickerAdding") : t("pages.favourites.pickerAdd")}
            </button>
          </div>
        </div>
      </div>
