  ...
  const tagOverlay = card.tags.length ? (
    <div className="absolute bottom-3 right-3 flex max-w-[calc(100%-1.5rem)] flex-wrap justify-end gap-1.5">
      {card.tags.slice(0, 3).map((tag) => (
        <span key={tag} className="rounded-full bg-black/60 px-2.5 py-1 text-[11px] font-semibold text-white shadow-md backdrop-blur-sm">
          {tag}
        </span>
      ))}
    </div>
  ...
  return (
    ...
    >
      ...
      {contextMenu ? (
        <div
          ref={menuRef}
          className="fixed z-50 min-w-[160px] rounded-lg border border-border bg-surface p-1 shadow-2xl"
          style={{ left: contextMenu.x, top: contextMenu.y }}
        >
          <button
            type="button"
            className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-foreground transition hover:bg-surface-strong"
            onClick={() => {
              openCard();
              setContextMenu(null);
            }}
          ...
          {onFavorite ? (
            ...
            >
              <Star size={15} className={card.isFavorite ? "fill-accent text-accent" : ""} />
              {card.isFavorite ? t("toolCard.toggleFavourite") : t("toolCard.toggleFavourite")}
            </button>
          ) : null}
        </div>
      ) : null}
